...
class Slave implements Runnable {
	...
	public void connect(String[] command) {
		if (!command[0].equalsIgnoreCase("connect") || command.length < 4) {
			return;
		}

		String targetIP = command[2];
		String targetPort = command[3];
		String numberOfConnections = "1";
		if (command.length == 5) {
			numberOfConnections = command[4];
		}
		out.println("connect " + targetIP + " " + targetPort + " " + numberOfConnections);
	}

	public void disconnect(String[] command) {
		if (!command[0].equalsIgnoreCase("disconnect") || command.length < 3) {
			return;
		}
		...
		String targetPort = "";
		if (command.length == 4) {
			targetPort = command[3];
		}

		out.println("disconnect " + targetIP + " " + targetPort);
	}
